...
_module = load_personal_module("task1_collect_legal_docs")

# Expose everything to the root namespace
globals().update({k: v for k, v in _module.__dict__.items() if not k.startswith('_')})

if __name__ == "__main__":
    if hasattr(_module, "collect_docs"):
        _module.collect_docs()
    elif hasattr(_module, "download_all"):
        _module.download_all()
